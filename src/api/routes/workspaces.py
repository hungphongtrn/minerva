...
class WorkspaceBootstrapResponse(BaseModel):
    ...
    created: bool = Field(..., description="True if workspace was created, False if reused")
...
class SandboxResolveResponse(BaseModel):
    ...
    sandbox_id: Optional[str] = Field(None, description="Sandbox identifier if resolved")
    ...
    ttl_cleanup_applied: bool = Field(False, description="Whether idle TTL cleanup was applied")
    ttl_stopped_count: int = Field(0, description="Number of sandboxes stopped due to idle TTL")
...
async def resolve_sandbox(
    ...
    if not is_guest_principal(principal):
        ...
        return SandboxResolveResponse(
            ...
            ttl_cleanup_applied=getattr(target.routing_result, "ttl_cleanup_applied", False),
            ttl_stopped_count=len(getattr(target.routing_result, "stopped_sandbox_ids", []) or []),
            ttl_stopped_ids=getattr(target.routing_result, "stopped_sandbox_ids", None),
            ttl_cleanup_reason=getattr(target.routing_result, "ttl_cleanup_reason", None),
        )
