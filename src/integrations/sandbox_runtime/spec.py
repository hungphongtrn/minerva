"""Sandbox runtime spec loader and validator.

Provides Pydantic v2 models and loader for sandbox runtime gateway integration specs.
"""
...
class RuntimePathsSpec(BaseModel):
    """Runtime path and command configuration."""
...
class SandboxRuntimeSpec(BaseModel):
    """Complete sandbox runtime gateway integration specification."""
    ...
    runtime: RuntimePathsSpec = Field(..., description="Runtime configuration")
    examples: ExamplesSpec = Field(..., description="Example request/response payloads")


def load_runtime_spec(
    path: str = "src/integrations/sandbox_runtime/spec.json",
) -> SandboxRuntimeSpec:
    """Load and validate a sandbox runtime spec file.

    Args:
        path: Path to the spec JSON file.

    Returns:
        Validated SandboxRuntimeSpec instance.
    ...
    if not spec_path.exists():
        raise FileNotFoundError(f"Sandbox runtime spec not found: {path}")

    with open(spec_path, encoding="utf-8") as f:
        data = json.load(f)

    return SandboxRuntimeSpec.model_validate(data)
