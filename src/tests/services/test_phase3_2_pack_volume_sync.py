...
class TestVolumeNaming:
    ...
    def test_volume_name_format(self, pack_id, source_digest):
        """Volume name follows agent-pack-{id}-{digest} pattern."""
        service = DaytonaPackVolumeService()
        volume_name = service._compute_volume_name(pack_id, source_digest)
        
        # Should match pattern: agent-pack-{first 8 of id}-{first 16 of digest}
        expected_prefix = f"agent-pack-{str(pack_id)[:8]}-{source_digest[:16]}"
        assert volume_name == expected_prefix
...
class TestSyncPackToVolume:
    ...
    async def test_sync_creates_volume(self, pack_id, source_digest, pack_files):
        ...
        mock_volume.name = f"agent-pack-{str(pack_id)[:8]}-{source_digest[:16]}"
        ...
        with patch("src.services.daytona_pack_volume_service.AsyncDaytona") as mock_daytona_cls:
            ...
            expected_name = f"agent-pack-{str(pack_id)[:8]}-{source_digest[:16]}"
    ...
    async def test_sync_uploads_to_correct_destination(self, pack_id, source_digest, pack_files):
        ...
        with patch("src.services.daytona_pack_volume_service.AsyncDaytona") as mock_daytona_cls:
            ...
            call_kwargs = mock_sandbox.fs.upload_files.call_args[1]
            assert call_kwargs["dest_dir"] == "/workspace/pack"
    ...
    @pytest.mark.asyncio
    async def test_sync_creates_sandbox_with_volume_mount(self, pack_id, source_digest, pack_files):
        """Disposable sandbox is created with volume mounted at /workspace/pack."""
        service = DaytonaPackVolumeService()
        
        expected_volume_name = f"agent-pack-{str(pack_id)[:8]}-{source_digest[:16]}"
        ...
        with patch("src.services.daytona_pack_volume_service.AsyncDaytona") as mock_daytona_cls:
            ...
            # Verify sandbox was created with volume mount
            mock_daytona.create.assert_called_once()
            call_kwargs = mock_daytona.create.call_args[1]
            
            assert "volumes" in call_kwargs
            assert len(call_kwargs["volumes"]) == 1
            
            volume_mount = call_kwargs["volumes"][0]
...
class TestVolumeMount:
    ...
    def test_volume_mount_attributes(self):
        """VolumeMount stores volume_id and mount_path."""
        mount = VolumeMount(
            volume_id="test-volume",
            mount_path="/workspace/pack",
        )
        
        assert mount.volume_id == "test-volume"
        assert mount.mount_path == "/workspace/pack"
